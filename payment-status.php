...
<div class="ps-card">
    ...
    <div class="ps-failure" id="ps-failure">
        ...
        <a href="/mumbai-darshan-bus-booking/" class="ps-retry-btn">🔄 Try Again</a>
    </div>
</div>
...
if (psOrderId) {
    ...
    .then(data => {
        ...
        if (data.order_status === 'PAID') {
            ...
            // Send admin email — only once per order
            if (!localStorage.getItem('email_sent_' + psOrderId)) {
                fetch('/send-mail.php', {
                    method: 'POST',
                ...
                .then(res => {
                    ...
                    localStorage.setItem('email_sent_' + psOrderId, 'true');
                })
...
function psDownloadReceipt() {
    ...
    const details = [
        ...
        ['Pickup', psPickup], ['Package', psPkg], ['Passengers', psSeats],
        ['Total Paid', '\u20b9' + psTotal]
    ];
    let y = 190;
